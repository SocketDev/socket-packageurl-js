#!/usr/bin/env node
// Claude Code PreToolUse hook — gitignore-is-single-file-at-edit.
//
...
export const check = editGuard((filePath, content, payload) => {
  ...
  return block(
    [
      'gitignore-is-single-file-at-edit: this write would create a nested `.gitignore` - every fleet repo keeps ignore rules in ONE root `.gitignore`.',
      `Where: ${rel}`,
      `Fix: add the pattern to the root .gitignore using a \`**/\`-anchored line, e.g. \`**/${rel.replace(/\/\.gitignore$/, '')}/<file>\`.`,
    ].join('\n'),
  )
})
