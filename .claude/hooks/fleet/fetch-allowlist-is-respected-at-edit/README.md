...
matcher live in `_shared/fetch-allowlist.mts`, shared with the commit-time check
(`scripts/fleet/check/fetch-allowlist-is-respected-at-commit.mts`) so the two never drift
(code is law, DRY).
...
`Allow fetch-allowlist bypass` in a recent user turn, for a one-off legitimate
fetch. To permanently allow a new PUBLIC registry/CDN, add it to
`SNAPSHOT_HOSTS` in `_shared/fetch-allowlist.mts`.
