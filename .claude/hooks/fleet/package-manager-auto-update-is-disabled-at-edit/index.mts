#!/usr/bin/env node
// Claude Code PreToolUse(Bash) hook — package-manager-auto-update-is-disabled-at-edit.
//
...
export const check = bashGuard((command, payload) => {
  ...
  return block(
    [
      verdictLine(
        'block',
        'package-manager-auto-update-is-disabled-at-edit',
        `\`${invoked.binaries[0]}\` blocked - ${invoked.id} auto-update is enabled (${status.reason}).`,
      ),
      `Fix: ${status.fix} - or run node .claude/hooks/fleet/setup-security-tools/install.mts`,
