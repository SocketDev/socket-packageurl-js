# markdown-filenames-are-canonical-at-edit
...
The fleet's `scripts/fleet/check/markdown-filenames-are-canonical-at-commit.mts` does the same check at commit time; this hook catches it earlier, at edit time, so the model gets immediate feedback when it picks a wrong name.
...
The hook fails open on its own bugs (exit 0 + stderr log) so a bad deploy can't brick the session. The `scripts/fleet/check/markdown-filenames-are-canonical-at-commit.mts` gate at commit time is the second line of defense.
