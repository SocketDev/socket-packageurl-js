#!/usr/bin/env node
// Claude Code PreToolUse hook — markdown-filenames-are-canonical-at-edit.
//
...
// the commit-time belt check `scripts/fleet/check/markdown-filenames-are-canonical-at-commit.mts`;
...
export function emitBlock(filePath: string, verdict: Verdict): string {
  ...
  return verdictLine(
    ...
    `blocked non-canonical doc filename "${filePath}" — ${issue}${fix}\n`,
  )
}
