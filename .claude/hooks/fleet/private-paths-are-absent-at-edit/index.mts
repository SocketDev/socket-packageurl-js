#!/usr/bin/env node
// Claude Code PreToolUse hook — private-paths-are-absent-at-edit.
//
...
export const check = editGuard((filePath, content) => {
  ...
  const lines: string[] = [
    verdictLine(
      ...
      `blocked ${describePrivatePathKind(first.kind)} "${first.match}" in a comment (line ${first.line} of ${filePath}) — remove the path; describe the constraint, not where a note lives`,
    ),
  ]
