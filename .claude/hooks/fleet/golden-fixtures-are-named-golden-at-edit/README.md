...
- Fires only on **creation** of a `*.expected.json` name. Editing one that
  already exists on disk, whether a pre-rule fixture or a rename in progress,
  is never blocked.
- Fleet-only (`isFleetTarget`): an external / sibling clone owns its own naming.
- Belt scan: `scripts/fleet/check/golden-fixtures-are-named-golden-at-commit.mts` fails
