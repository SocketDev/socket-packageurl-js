#!/usr/bin/env node
// Claude Code PreToolUse hook — workflow-sha-pins-are-stamped-at-edit.
//
...
//     are checked.
//   - Lines marked `# oxlint-disable-next-line socket/workflow-uses-has-stamp` are exempt for
...
export function findBadUsesLines(text: string): BadLine[] {
  ...
  for (let i = 0, { length } = lines; i < length; i += 1) {
    ...
    if (lineIsSuppressed(line, 'workflow-uses-has-stamp')) {
      continue
    }
...
// A generator's output announces itself: a `DO NOT EDIT` header near the top.
// A generator can emit a plain `.yml` the path exemption misses, and its
// `uses:` comments are the generator's own (a bare version, no date), so the
// canonical stamp cannot be asked of it. Mirrored in the sync-scaffolding
// check `workflow-uses-comment.mts`.
function isGeneratedWorkflow(content: string): boolean {
  const head = content.slice(0, 2000)
  return head.includes('DO NOT EDIT') && /generated by/i.test(head)
}
...
export const check = editGuard((filePath, content) => {
  ...
  if (isGeneratedWorkflow(proposed)) {
    return undefined
  }
  ...
  return block(
    `workflow-sha-pins-are-stamped-at-edit: refusing edit: ${bad.length} ` +
      ...
      '\nOne-off override: add `# oxlint-disable-next-line socket/workflow-uses-has-stamp` on its own line above\n' +
      'to the `uses:` line.',
  )
})
