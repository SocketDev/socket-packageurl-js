...
// edit-time guard AND `check/rule-citations-are-generic-at-commit.mts` (the commit-time
...
const SELF_EXEMPT_FRAGMENTS = [
  'hooks/fleet/rule-citations-are-generic-at-edit/',
...
export const check = editGuard((filePath, content, payload) => {
  ...
  return block(
    [
      `rule-citations-are-generic-at-edit: blocked a dated-incident citation added to rule prose - dates and SHAs age into a changelog.`,
      `Where: ${filePath} - ${evidence}`,
      `Fix: rewrite the citation as a timeless example (drop the date / version delta / percentage / SHA).`,
    ].join('\n'),
  )
})
