#!/usr/bin/env node
// Claude Code Stop hook — error-messages-are-thorough-at-edit.
//
...
// `error-messages-are-thorough-at-commit` check grade identically. AST-based detection:
...
export const check = (payload: ToolCallPayload): GuardResult => {
  ...
  for (let i = 1, { length } = findings; i < length; i += 1) {
    ...
    lines.push(
      verdictContinuation(
        `Vague: throw new ${f.errorClass}("${f.message}") - ${f.label}`,
      ),
    )
  }
