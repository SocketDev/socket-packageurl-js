...
- `splitPurlPackageName(type, packageName)` splits a raw ecosystem package name into its `{ namespace, name }` components per the PURL type rules: composer and openvsx take the vendor before the first slash, golang the module path before the last slash, maven the `groupId:artifactId` pair, npm the scope, and namespace-less types keep the whole name. Use it instead of hand-rolling a per-type split, which tends to drop the namespace for a forgotten type.
