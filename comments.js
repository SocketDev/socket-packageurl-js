 ...
 */
{
  ...
  const renderAll = () => {
    ...
    const groups = state.comments.reduce((acc, c) => {
      const k = `${c.file}:${c.lineFrom}`
      ;(acc.get(k) ?? (acc.set(k, []), acc.get(k))).push(c)
      return acc
    }, new Map())
    ...
    for (const [key, group] of groups) {
      ...
      const byParent = group
        .filter(c => c.parentId)
        .reduce((acc, c) => {
          ;(
            acc.get(c.parentId) ??
            (acc.set(c.parentId, []), acc.get(c.parentId))
          ).push(c)
          return acc
        }, new Map())
