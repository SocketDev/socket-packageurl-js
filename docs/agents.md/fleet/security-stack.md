...
| Hook bypass attempts             | `.claude/hooks/fleet/no-revert-guard/`                                                                              | Blocks work-losing git (`checkout`/`clean`/`reset --hard`/`restore`/`rm`/`stash`), `--no-verify`, `HUSKY=0`, a redirected `core.hooksPath`, `--no-gpg-sign` — gated by canonical `Allow X bypass` phrases. `git revert` appends a commit and loses nothing, so it is a nudge, not a block. |
| Force-push attempts              | `.claude/hooks/fleet/no-force-push-guard/`                                                                          | Blocks `git push --force`/`-f`/`--force-with-lease`/`--force-if-includes`; gated by `Allow force-push bypass` (legacy aliases `force-with-lease bypass`, `force-push-hard bypass`)                                                                                                         |
...
| GitHub repo settings         | `scripts/fleet/lint-github-settings.mts`             | Audits visibility, merge settings, branch protection, required apps. Weekly cache-gated; CI doesn't burn API quota                                                                                                        |
...
| Hook-chain skip                        | `.claude/hooks/fleet/no-revert-guard/`                         | `--no-verify`, `HUSKY=0`, or a redirected `core.hooksPath`, without the typed bypass phrase |
