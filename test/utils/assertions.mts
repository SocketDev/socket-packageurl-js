 ...
 *
 * @example
 *   testThrowingAndNonThrowing(() => validateName('invalid name'), {
 *     shouldPass: false,
 *     errorPattern: /invalid name format/,
 *   })
 */
export function testThrowingAndNonThrowing(
  validator: (_opts: { throws: boolean }) => boolean | never,
  options: { shouldPass: boolean; errorPattern?: RegExp | string | undefined },
): void {
  const { errorPattern, shouldPass } = options
  // Non-throwing mode
