/**
 * @file Tests for NuGet registry existence checks.
 */
...
describe('nugetExists catalog-entry gap cases', () => {
  beforeEach(() => {
    nock.disableNetConnect()
  })
  ...
  it('skips a catalog entry with no version instead of pushing it', async () => {
    nock('https://api.nuget.org')
      .get('/v3/registration5-semver1/gapcase/index.json')
      .reply(200, {
        items: [
          {
            items: [
    ...
    const result = await nugetExists('gapcase')

    expect(result).toEqual({ exists: true, latestVersion: '1.0.0' })
  })
})
