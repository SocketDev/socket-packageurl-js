...
export function hostOf(url: string): string {
  try {
    ...
    return at >= 0 && colon > at ? url.slice(at + 1, colon).toLowerCase() : ''
  }
}
...
export function ruleForHost(host: string): HostRule | undefined {
  for (let i = 0, { length } = HOST_RULES; i < length; i += 1) {
    const rule = HOST_RULES[i]!
...
export function ruleForShorthand(url: string): HostRule | undefined {
  for (let i = 0, { length } = HOST_RULES; i < length; i += 1) {
    const rule = HOST_RULES[i]!
 ...
 */
export function stripGitProtocol(url: string): string {
