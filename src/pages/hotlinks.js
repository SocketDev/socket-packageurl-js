...
;(() => {
  ...
  const installSourceLinks = () => {
    ...
    const resolveRelPath = (fromPath, ref) => {
      ...
      // oxlint-disable-next-line socket/normalize-path-before-match -- `fromPath` is a site URL path in a browser page script, always '/'-separated; never a platform path.
    ...
    // Quoted relative path: group 1 captures the opening quote (' or "), group 2
    // the `./` or `../` path body (no quotes/backtick), and \1 backreferences the
    // same closing quote.
