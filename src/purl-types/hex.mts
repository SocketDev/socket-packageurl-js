...
export async function hexExists(
  ...
  const fetchResult = async (): Promise<ExistsResult> => {
    try {
      const url = `https://hex.pm/api/packages/${GlobalEncodeUriComponent(name)}`
      ...
      if (version) {
        ...
        const versionExists = ArrayPrototypeSome(
          releases,
          (r: { version?: string | undefined }) => r.version === version,
        )
        if (!versionExists) {
...
export function validate(
  ...
  if (
    !validateNoInjectionByType('hex', 'namespace', purl.namespace, { throws })
  ) {
    return false
  }
