...
export async function runClaude(
  ...
  try {
    ...
    modelStrategy.recordAttempt(task, { success: true })

    return result
  } catch (e) {
    // Record failure for potential escalation
    modelStrategy.recordAttempt(task, { success: false })
...
export async function syncClaudeMd(
  ...
  if (shouldRunParallel(opts) && otherProjects.length > 1) {
    ...
    // Run sequentially
    for (let i = 0, { length } = otherProjects; i < length; i += 1) {
...
Remember: small commits, follow project standards, no AI attribution.`
    ...
    // Run sequentially
    for (let j = 0, { length: jl } = projects; j < jl; j += 1) {
...
Commit message:`
  ...
  const result = await new Promise((resolve, reject) => {
    ...
    const child = claudeProcess.process
...
Let's work through this together to get CI passing.`
  ...
  // Parse a GitHub remote: after `github.com` + `:` or `/`, capture owner (1)
  // and repo (2), with an optional trailing `.git` (3) stripped.
      ...
      if (jobsResult.exitCode === 0 && jobsResult.stdout) {
        try {
          ...
          if (newFailures.length > 0) {
            ...
            for (let k = 0, { length: kl } = sortedFailures; k < kl; k += 1) {
              const job = sortedFailures[k]
