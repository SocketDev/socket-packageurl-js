 ...
 */
export function memoryStoreDir(repoRoot: string, home = CLAUDE_HOME): string {
  const slug = repoRoot.replaceAll('\\', '-').replaceAll('/', '-')
  return path.join(home, 'projects', slug, 'memory')
}
 ...
 *
 * The matcher itself lives in the hook `_shared/memory-store.mts`, shared with
 * `memories-are-codified-at-edit` (the write-time block) so this commit-time
 * belt and that guard can never disagree about what counts as a stamp.
 */
export function isCodified(content: string): boolean {
  return hasEnforcementStamp(content)
}
...
export function memoryType(content: string): string | undefined {
  return memoryFrontmatterType(content)
}
...
export function main(
  options?: { claudeHome?: string | undefined } | undefined,
): void {
  const { claudeHome = CLAUDE_HOME } = { __proto__: null, ...options } as {
    claudeHome?: string | undefined
  }
  const repoRoot = process.env['CLAUDE_PROJECT_DIR'] ?? REPO_ROOT
  const dir = memoryStoreDir(repoRoot, claudeHome)
  if (!existsSync(dir)) {
    logger.log(
      'memories-are-codified-at-commit: skipped (no memory store for this project — CI / fresh checkout).',
    )
    return
  }
  ...
  if (uncodified.length === 0) {
    logger.log(
      `memories-are-codified-at-commit: OK — all ${candidates} codifiable memories carry an enforcement: disposition.`,
    )
    return
  }
  logger.warn(
    `memories-are-codified-at-commit: ${uncodified.length}/${candidates} codifiable memories are UNCODIFIED ` +
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: 'Usage: node scripts/fleet/check/memories-are-codified-at-commit.mts',
}
