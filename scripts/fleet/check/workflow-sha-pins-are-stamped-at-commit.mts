 ...
 *   (`.claude/hooks/fleet/workflow-sha-pins-are-stamped-at-edit/`) is a PreToolUse guard,
 ...
 *     scripts/fleet/check/workflow-sha-pins-are-stamped-at-commit.mts [--quiet]
 */
...
export function trackedPinStampedSurfaces(rootDir: string): string[] {
  const result = gitSync(['ls-files', '*.yml', '*.yaml'], {
    cwd: rootDir,
  })
  if (result.status !== 0) {
    return []
...
function main(): void {
  ...
  logger.fail(
    `[workflow-sha-pins-are-stamped-at-commit] ${findings.length} SHA pin(s) missing the canonical stamp.`,
  )
  logger.group()
  logger.error(
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/workflow-sha-pins-are-stamped-at-commit.mts [flags]
