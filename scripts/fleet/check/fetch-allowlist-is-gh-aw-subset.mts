 ...
 *   (.config/fleet/fetch-allowlist.json) is a SUBSET of gh-aw's expanded
...
function listGhAwLocks(): string[] {
  try {
    const r = gitSync(['ls-files', '*.github/workflows/*.lock.yml'], {})
    if (r.status !== 0) {
      return []
    }
    const { stdout } = r
...
if (!existsSync(allowlistPath)) {
  ...
  try {
    ...
    mode = parsed.mode
    // The runtime-scoped subset of the ONE `hosts[]` list, read through the
    // shared reader the guard uses. Reading a second array here is what let a
    // host be granted at runtime while escaping this containment check.
    const scoped = hostsForScope(parsed, 'runtime')
    if (!scoped) {
      logger.fail(
        `[fetch-allowlist] malformed ${ALLOWLIST_REL}: "hosts" must be an ` +
          `array of { host, scopes } entries. ` +
    ...
    logger.fail(
      `[fetch-allowlist] could not parse ${ALLOWLIST_REL}: ${String(e)}. ` +
  ...
  if (parsedOk) {
    ...
    if (ghHosts.size === 0) {
      ...
      if (escapes.length === 0) {
        logger.success(
          `[fetch-allowlist] ${fleetHosts.length} host(s) subset-of gh-aw ` +
            `firewall allowDomains (${ghHosts.size} trusted); mode="${String(mode)}".`,
        )
        process.exitCode = 0
      } else {
        logger.fail(
          `[fetch-allowlist] ${escapes.length} host(s) in ${ALLOWLIST_REL} are ` +
