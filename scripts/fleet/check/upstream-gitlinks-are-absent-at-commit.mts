 ...
 *   update-index --force-remove`, which keeps `.gitmodules`; a gitlink is
 *   exactly the entry `git rm --cached` mishandles — via
 *   _shared/untrack-offenders.mts) and RE-RUNS the detection — the re-check,
 *   not the executor, decides the exit. Exit: 0 — no tracked upstream gitlink,
 *   or git is unavailable; 1 — at least one `160000` entry under `upstream/`
 *   (or residual after `--fix`). Usage: node
 *   scripts/fleet/check/upstream-gitlinks-are-absent-at-commit.mts [--fix]
 *   [--quiet]
 */
...
export function findTrackedUpstreamGitlinks(
  ...
  const out: string[] = []
  const lines = lsFilesStageOutput.split(/\r?\n/)
...
export async function main(): Promise<void> {
  ...
  if (offenders.length > 0 && fix) {
    ...
    for (let i = 0, { length } = failures; i < length; i += 1) {
      const f = failures[i]!
      logger.warn(
        `upstream-gitlinks-are-absent-at-commit: fix step failed: ${formatUntrackAction(f.action)} — ${f.detail}`,
      )
    }
  ...
  if (offenders.length === 0) {
    if (!process.argv.includes('--quiet')) {
      logger.log(
        'upstream-gitlinks-are-absent-at-commit: no gitlink tracked under upstream/.',
      )
    }
    process.exitCode = 0
    return
  ...
  logger.fail(
    `upstream-gitlinks-are-absent-at-commit: ${offenders.length} gitlink(s) tracked under upstream/${fix ? ' after --fix' : ''}:`,
  )
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/upstream-gitlinks-are-absent-at-commit.mts [flags]
