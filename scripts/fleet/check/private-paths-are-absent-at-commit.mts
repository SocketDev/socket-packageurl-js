...
//
// The commit-time complement to the `private-paths-are-absent-at-edit` edit-time
...
// commit-time hook uses — one pattern set and one scope, no drift. A private
// copy of that scope here lets generated JSON fail the hook while passing this
// gate.
//
// Usage: node scripts/fleet/check/private-paths-are-absent-at-commit.mts [--quiet]
...
const SELF_EXEMPT_SUBSTRINGS: readonly string[] = [
  ...
  'check-private-paths-are-absent-at-commit.test.mts',
  'check/private-paths-are-absent-at-commit.mts',
  'private-paths-are-absent-at-edit.test.mts',
  'private-paths-are-absent-at-edit/',
...
export function main(): void {
  ...
  if (hits.length) {
    logger.fail(
      '[check-private-paths-are-absent-at-commit] private/internal paths in source comments:',
    )
  ...
  if (!quiet) {
    logger.success(
      '[check-private-paths-are-absent-at-commit] no private paths in source comments.',
    )
  }
}
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/private-paths-are-absent-at-commit.mts [flags]
