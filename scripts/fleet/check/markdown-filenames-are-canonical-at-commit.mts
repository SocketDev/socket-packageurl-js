 ...
 *   Commit-time twin of the edit-time markdown-filenames-are-canonical-at-edit
 *   hook: the guard blocks a bad NEW filename at Bash time; this catches one
 *   already committed or introduced off the Claude path. REUSES the guard's
...
export interface MarkdownViolation {
  file: string
  message: string
  // The canonical name to rename to, when the classifier can derive one. A Fix
  // line that says "rename it" without naming the target leaves the reader to
  // guess the rule back out of the message.
...
export function trackedMarkdownFiles(rootDir: string): string[] {
  const result = gitSync(['ls-files', '*.md', '*.markdown', '*.MD'], {
    cwd: rootDir,
  })
  if (result.status !== 0) {
    return []
...
export async function main(): Promise<void> {
  ...
  for (const violation of violations) {
    ...
    if (violation.suggestion) {
      logger.log(`    Fix: rename to ${violation.suggestion}`)
    }
  }
  process.exitCode = 1
}
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: 'Usage: node scripts/fleet/check/markdown-filenames-are-canonical-at-commit.mts',
}
