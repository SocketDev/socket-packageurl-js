...
//
// The edit-time `env-kill-switches-are-absent-at-edit` hook blocks NEW writes that
...
//
// Self-exempt: the `env-kill-switches-are-absent-at-edit` hook itself (its source + README +
// tests legitimately name the patterns it bans) and this check's own test.
//
// Usage: node scripts/fleet/check/env-kill-switches-are-absent-at-commit.mts [--quiet]
...
const SELF_EXEMPT_HOOKS = new Set(['env-kill-switches-are-absent-at-edit'])
...
export function main(): void {
  ...
  if (hits.length) {
    logger.fail(
      '[check-env-kill-switches-are-absent-at-commit] env kill-switch references in the hook tree:',
    )
    for (let i = 0, { length } = hits; i < length; i += 1) {
      const h = hits[i]!
      logger.fail(`  ${h.file}:${h.line} — ${h.text}`)
    }
    logger.error(
  ...
  if (!quiet) {
    logger.success(
      '[check-env-kill-switches-are-absent-at-commit] no env kill-switches in the hook tree.',
    )
  }
}

const SCRIPT_META: ScriptMeta = {
  describe: 'checks the hook tree carries no env kill-switches',
  help: `Usage: node scripts/fleet/check/env-kill-switches-are-absent-at-commit.mts [flags]
