...
//
// Commit-time complement to the `rule-citations-are-generic-at-edit` PreToolUse hook. The
...
//
// Scope: the fleet-facing rule-prose surfaces — CLAUDE.md, docs/fleet/agents.md,
...
//
// Usage: node scripts/fleet/check/rule-citations-are-generic-at-commit.mts [--quiet]
...
const SELF_EXEMPT_FRAGMENTS = [
  ...
  'rule-citations-are-generic-at-edit/',
  'check/rule-citations-are-generic-at-commit',
]
...
export function main(): void {
  ...
  if (findings.length) {
    logger.fail(
      '[check-rule-citations-are-generic-at-commit] dated-incident citations in rule prose — rewrite generically, as a timeless example (drop dates / version deltas / percentages / SHAs):',
    )
    for (let i = 0, { length } = findings; i < length; i += 1) {
      const f = findings[i]!
      logger.fail(`  ${f.file}:${f.line} — ${f.label}`)
      logger.error(`      ${f.text}`)
    }
    process.exitCode = 1
    return
  ...
  if (!quiet) {
    logger.success(
      '[check-rule-citations-are-generic-at-commit] all rule citations are generic.',
    )
  }
}
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/rule-citations-are-generic-at-commit.mts [flags]
