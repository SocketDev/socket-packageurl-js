...
//
// Commit-time complement to the `error-messages-are-thorough-at-edit` Stop hook. The
...
// pattern as env-kill-switches-are-absent-at-edit / env-kill-switches-are-absent-at-commit.
//
...
//
// Usage: node scripts/fleet/check/error-messages-are-thorough-at-commit.mts [--quiet]
...
const SELF_EXEMPT_FRAGMENTS = [
  ...
  'check/error-messages-are-thorough-at-commit',
]
...
export function main(): void {
  ...
  if (hits.length) {
    logger.fail(
      '[check-error-messages-are-thorough-at-commit] vague-only error messages (state what / where / saw-vs-wanted / fix):',
    )
  ...
  if (!quiet) {
    logger.success(
      '[check-error-messages-are-thorough-at-commit] no vague-only error messages.',
    )
  }
}

const SCRIPT_META: ScriptMeta = {
  describe: 'checks the committed source tree for vague-only error messages',
  help: `Usage: node scripts/fleet/check/error-messages-are-thorough-at-commit.mts [flags]
