 ...
 *   it drifts per machine; this gate catches the drift. Shares ALL detection
 *   logic with the point-of-use
 *   `.claude/hooks/fleet/package-manager-auto-update-is-disabled-at-edit/` and
 *   the `setup-security-tools` installer via
 ...
 *   every installed manager has auto-update disabled, or none installed; 1 — at
...
if (enabled.length === 0) {
  logger.log(
    'package-manager auto-update: disabled on every installed manager.',
  )
  process.exitCode = 0
} else {
  logger.error('')
