 ...
 *   `*.golden.json` target. Reuses the `goldenTarget` predicate from the
...
export function trackedExpectedJsonFiles(rootDir: string): string[] {
  const result = gitSync(['ls-files', '*.expected.json'], {
    cwd: rootDir,
  })
  if (result.status !== 0) {
    return []
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: 'Usage: node scripts/fleet/check/golden-fixtures-are-named-golden-at-commit.mts',
}
