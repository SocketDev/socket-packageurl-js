 ...
 *   paired with the `socket/paths-are-normalized-before-match-at-edit` lint rule (write-time)
 ...
 *
 *   Usage: node scripts/fleet/check/paths-are-normalized-before-match-at-commit.mts [--quiet]
 */
...
// (socket/paths-are-normalized-before-match-at-edit) is the authoritative gate for anything
...
// The authoritative gate is the `socket/paths-are-normalized-before-match-at-edit` lint rule,
...
const LINT_SUPPRESSION_RE =
  /oxlint-disable-(?:next-)?line\b[^\n]*\bsocket\/paths-are-normalized-before-match-at-edit\b/
...
export function main(): void {
  ...
  for (let i = 0; i < files.length; i += 1) {
    ...
    if (file.endsWith('paths-are-normalized-before-match-at-commit.mts')) {
      continue
    }
  ...
  if (allFindings.length === 0) {
    if (!quiet) {
      process.stdout.write(
        '[paths-are-normalized-before-match-at-commit] all clean — no un-normalized path operations.\n',
      )
    }
    process.exit(0)
  }
  ...
  process.stderr.write(
    `[paths-are-normalized-before-match-at-commit] ${allFindings.length} un-normalized path operation${allFindings.length === 1 ? '' : 's'} found:\n`,
  )
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/paths-are-normalized-before-match-at-commit.mts [flags]
