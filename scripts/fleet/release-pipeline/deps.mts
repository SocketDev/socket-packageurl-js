 ...
 *   shape every runner returns, the injectable process/registry dependencies
...
import type { NpmIdentityReport } from '../registry-infra/npm/auth-identity.mts'
import {
  ensureTagAndRelease,
  requireRegistryLive,
} from '../registry-infra/release.mts'
import { listStagedPackages } from '../registry-infra/npm/shared.mts'
import {
  fetchVersionTrustInfo,
  isAlreadyPublished,
} from '../registry-infra/npm/registry.mts'
import {
  preflightSocketScanAuth,
  scanStagedEntryDetailed,
} from '../registry-infra/npm/scan.mts'
import {
...
import {
  ...
  runInherit,
  runPtyPumped,
} from '../registry-infra/shared.mts'

import type {
  SocketScanContext,
  StagedScanVerdict,
} from '../registry-infra/npm/scan.mts'
import type { StageListEntry } from '../registry-infra/npm/shared.mts'
import type { TarballProvider } from '../registry-infra/npm/staged.mts'
...
/**
 * Injectable process/registry dependencies. Defaults are the real
 * registry-infra helpers; tests inject fakes so no runner ever spawns for real.
 */
export interface RunnerDeps {
  ...
  fetchRegistryDist?:
    | ((
        name: string,
        // oxlint-disable-next-line socket/prefer-refined-record -- registry doc
...
function defaultFetchRegistryDist(
  name: string,
  // The abbreviated packument's versions map is the registry's JSON document.
  // oxlint-disable-next-line socket/prefer-refined-record -- packument doc
): Promise<Record<string, RegistryDistInfo>> {
