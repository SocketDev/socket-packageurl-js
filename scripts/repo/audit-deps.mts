...
let sdk: SocketSdk | undefined

// Lazy so importing this module never captures a native handle at eval time.
function getSdk(): SocketSdk {
  if (sdk === undefined) {
    sdk = new SocketSdk(SOCKET_PUBLIC_API_TOKEN, { timeout: API_TIMEOUT_MS })
  }
  return sdk
}
...
export async function checkMalwareBatched(
  ...
  for (
    let offset = 0, { length: total } = deps;
    offset < total;
    offset += MAX_BATCH_SIZE
  ) {
    const batch = deps.slice(offset, offset + MAX_BATCH_SIZE)
    const components = batch.map(d => ({
      purl: `pkg:npm/${d.name}@${d.version}`,
    }))
    const result = await getSdk().checkMalware(components)
    if (!result.success) {
      throw new Error(
