  ...
  meta: {
    ...
    messages: {
      normalizeBeforeRegexMatch:
        "Path-like '{{name}}' in a regex match; wrap in `normalizePath({{name}})` from @socketsecurity/lib/paths/normalize.",
      normalizeBeforeStringSep:
        "Path-like '{{name}}' in a separator op; wrap in `normalizePath({{name}})` from @socketsecurity/lib/paths/normalize.",
    },
    schema: [],
  },
